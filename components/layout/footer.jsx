			...
			<Container spacing={['verticalXXLrg', 'bottomLrg']}>
				<section className={css.sections}>
					<ul className={css.thanks}>
						...
						{
						content.experience.map( ({ person, link, note }, index) => {
							return (
								<li key={index}>
					...
					<ul className={css.links}>
						<li><h4>Latest Certificates</h4></li>
						{
						content.certificates.map( ({ person, link, note }, index) => {
							return (
								<li key={index}>
									<a href={link} rel="noreferrer" target="_blank">{person} <Icon icon={[ 'fad', 'arrow-up-right-from-square' ]} /></a>
									<p>{note}</p>
				...
				<section className={css.github}>
					<a href={settings.portfolio.repo_html} rel="noreferrer" target="_blank">
						...
						<ul>
							<li>
								<p><Icon icon={[ 'fad', 'code-branch' ]} /> PRs: { gitHubInfo.forks }</p>
							</li>
							<li>
								<p><Icon icon={[ 'fad', 'star' ]} /> Commits: { gitHubInfo.stars }</p>
							</li>
						</ul>
					</a>
				</section>
